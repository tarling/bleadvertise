...
define(['buffer-data-utils'], function(dataUtils) {
    ...
    function Packet(dataType, data, byteOrder) {
        ...
        if (data && !dataUtils.isBuffer(data)) {
    ...
    function toSignedInt(data) {
        if (!data) return 0;
        return dataUtils.readInt8(data, 0);
    }
    ...
    function toOctetString(numOctets, data, dataType, byteOrder) {
        ...
        for (var i = 0; i < numOctets; i++) {
            // Ready as hex string
            var str = dataUtils.readUInt8(data,i).toString(16);
    ...
    return {
        Packet: Packet,
        toOctetStringArray: toOctetStringArray
    }
